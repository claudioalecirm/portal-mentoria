...
export default async function handler(req, res) {
  ...
  if (isMesa) {
    const r = await resend.emails.send({
      ...
      to: email, subject: 'Bem-vindo à Mesa do Reino',
      html: emailMesa(email)
    })
    ...
    const r = await resend.emails.send({
      from: 'Claudio Alecrim <noreply@claudioalecrim.com.br>',
      to: email, subject: `Bem-vindo à ${mentoriaNome} — Complete seu cadastro`,
      html: emailOnboarding(mentoriaNome, link)
    })
...
function emailOnboarding(mentoria, link) {
  return `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#f5f5f5">
    ...
    <div style="background:#fff;padding:36px 40px">
      <p style="font-size:16px;color:#111;margin-bottom:12px">Olá!</p>
