...
export default function DashMentor({ user, onLogout, showPush }) {
  ...
  const carregarOverview = async () => {
    try {
      ...
      const temMsg = (ov.mensagensNaoLidas || []).length > 0
      setMsgDot(temMsg)
    } catch {}
  }
  ...
  const salvarEncontro = async () => {
    ...
    const payload = {
      ...
      resumo: editEnc.resumo, ferramentas_aplicadas: editEnc.ferramentas, tarefas_texto: editEnc.tarefas_texto,
  ...
  const inadimp = alunos.filter(a => a.progresso < 50)
        ...
        <div className="content">
          ...
          {alunoTab === 1 && (
            <div className="card">
              ...
              <div className="card-sub">Dados coletados via formulário</div>
              <div style={{ marginTop: 10 }}>
                {[
                  ['Nome', a?.usuarios?.nome], ['E-mail', a?.usuarios?.email], ['Telefone', a?.telefone],
                  ['Mentoria', a?.mentorias?.nome], ['Motivação', a?.motivacao], ['Resultado esperado', a?.resultado_esperado],
                  ['Área de foco', a?.area_foco], ['Grande desafio', a?.grande_desafio], ['Obstáculo', a?.obstaculo],
                  ['Satisfação atual', a?.satisfacao_atual !== null ? a?.satisfacao_atual + '/10' : '—'],
                  ['Tentativas anteriores', a?.tentativas_anteriores], ['Comprometimento', a?.comprometimento],
                  ['Visão ideal', a?.visao_ideal], ['Informações adicionais', a?.info_adicional]
                ].map(([label, val]) => (
          ...
          {alunoTab === 4 && (
            <Mensagens alunoId={alunoAtivo.id} perspectiva="mentor"
              onNovaMensagem={(msg) => showPush('Mensagem enviada', `Para: ${a?.usuarios?.nome}`)} />
          )}
      ...
      <div className="content">
        ...
        {tab === 0 && (
          <>
            <div className="card">
              ...
              <div className="card-sub">Abril 2026</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 9, marginTop: 10 }}>
                {[
                  { label: 'Ativos', val: alunos.length, color: 'var(--amber)' },
                  { label: 'Em dia', val: alunos.filter(a => a.acesso_ativo).length, color: 'var(--ok)' },
        ...
        {tab === 3 && (
          <div className="card">
            ...
            <div className="card-sub">Cadastrados via Diagnóstico Inicial</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginTop: 10 }}>
              {alunos.map(a => (
                <div key={a.id} className="li" onClick={() => abrirAluno(a)}>
                  <div className="li-avatar">{initials(a.usuarios?.nome)}</div>
