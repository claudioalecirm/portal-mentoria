...
export default function Onboarding() {
  ...
  const avancar = () => {
    ...
    if (step === 1 && !form.nome_preferido.trim()) { setErro('Informe como gostaria de ser chamado.'); return }
  ...
  const inp = { width: '100%', background: '#171717', border: '.5px solid rgba(255,255,255,.13)', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#f0ece4', fontFamily: "'DM Sans', sans-serif", outline: 'none', marginTop: 4, boxSizing: 'border-box' }
  const lbl = { fontSize: 11, color: '#5a5550', textTransform: 'uppercase', letterSpacing: '.08em', display: 'block', marginBottom: 2 }
  ...
  if (sucesso) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#0a0a0a', padding: 24 }}>
      <div style={{ background: '#111', border: '.5px solid rgba(200,169,122,.2)', borderRadius: 16, padding: '40px 32px', textAlign: 'center', maxWidth: 360 }}>
        ...
        <div style={{ fontSize: 13, color: '#9a9590', lineHeight: 1.6 }}>Redirecionando para o portal...</div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ background: '#111', border: '.5px solid rgba(255,255,255,.07)', borderRadius: 16, padding: '28px 24px', width: '100%', maxWidth: 460 }}>
        ...
        {step === 0 && (
          <div>
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: '#f0ece4', marginBottom: 12 }}>
              Bem-vindo!
            </div>
            <div style={{ fontSize: 13, color: '#9a9590', lineHeight: 1.7, marginBottom: 16 }}>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* Campo nome — captura o nome real */}
            <div>
              <label style={lbl}>Como gostaria de ser chamado?</label>
              <input style={inp} placeholder="Seu nome ou apelido" value={form.nome_preferido}
                onChange={e => set('nome_preferido', e.target.value)} />
            </div>

            <div>
              <label style={lbl}>Telefone / WhatsApp</label>
              <input style={inp} placeholder="(00) 00000-0000" value={form.telefone}
                onChange={e => set('telefone', e.target.value)} />
            </div>

            <div>
              <label style={lbl}>O que motivou você a entrar em um processo de mentoria agora?</label>
              <textarea style={{ ...inp, minHeight: 80, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="Seja específico sobre seu momento atual..."
                value={form.motivacao} onChange={e => set('motivacao', e.target.value)} />
            </div>

            <div>
              <label style={lbl}>Qual área da sua vida quer desenvolver com mais foco?</label>
              <select style={inp} value={form.area_foco} onChange={e => set('area_foco', e.target.value)}>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label style={lbl}>Qual é o principal resultado que você espera alcançar?</label>
              <textarea style={{ ...inp, minHeight: 70, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="Seja objetivo e específico..."
                value={form.resultado_esperado} onChange={e => set('resultado_esperado', e.target.value)} />
            </div>
            <div>
              <label style={lbl}>Se pudesse resolver apenas um grande desafio hoje, qual seria?</label>
              <textarea style={{ ...inp, minHeight: 70, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="O principal ponto que precisa mudar..."
                value={form.grande_desafio} onChange={e => set('grande_desafio', e.target.value)} />
            </div>
            <div>
              <label style={lbl}>O que você acredita que tem sido o maior obstáculo para avançar?</label>
              <textarea style={{ ...inp, minHeight: 70, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="Seja honesto consigo mesmo..."
                value={form.obstaculo} onChange={e => set('obstaculo', e.target.value)} />
            </div>
            <div>
              <label style={lbl}>Como você avalia sua satisfação atual nessa área? ({form.satisfacao_atual}/10)</label>
              <input type="range" min="0" max="10" value={form.satisfacao_atual}
                onChange={e => set('satisfacao_atual', e.target.value)}
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label style={lbl}>Você já tentou mudar ou melhorar essa área antes? Como foi?</label>
              <textarea style={{ ...inp, minHeight: 70, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="O que tentou e o que aprendeu com isso..."
                value={form.tentativas_anteriores} onChange={e => set('tentativas_anteriores', e.target.value)} />
            </div>
            <div>
              <label style={lbl}>Qual é o seu nível de comprometimento?</label>
            ...
            <div>
              <label style={lbl}>Se ao final dessa mentoria sua vida estivesse no nível que você deseja, como ela seria?</label>
              <textarea style={{ ...inp, minHeight: 80, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="Descreva com detalhes sua visão..."
                value={form.visao_ideal} onChange={e => set('visao_ideal', e.target.value)} />
            </div>
            <div>
              <label style={lbl}>Existe algo importante que eu deveria saber antes de iniciarmos? (opcional)</label>
              <textarea style={{ ...inp, minHeight: 60, resize: 'vertical', lineHeight: 1.5 }}
                placeholder="Contexto, situações específicas, pedidos especiais..."
                value={form.info_adicional} onChange={e => set('info_adicional', e.target.value)} />
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label style={lbl}>Criar senha (mínimo 6 caracteres)</label>
